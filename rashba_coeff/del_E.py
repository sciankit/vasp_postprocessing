...
ax.plot(x_values, y_values, label=f"del E [$K ⟶ \Gamma$]", marker=markers[0],color=colo[0])
ax.plot(x_values,y_values2, label=f"del T [$\Gamma ⟶ M$]", marker=markers[1],color=colo[1])
...
fit_line = np.polyfit(x_subset, y_subset, 2)
fit_line_fn = np.poly1d(fit_line)
a, b,c= fit_line
equation_str = f"{a:.3f}$x^2$ + {b:.3f}x + {c:.3f}"
...
ax.set_ylabel(f'$delta$ E (eV)')
ax.set_title(f'$delta$ E v/s Strain')
